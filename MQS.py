...
def Call_SAGA_Module(fDEM):            # pass your input file(s) here
    ...
    # ------------------------------------
    # call module: Modifed Quadratic Shepard
    Module = saga_api.SG_Get_Module_Library_Manager().Get_Module('grid_gridding','4')

    Parms = Module.Get_Parameters() # default parameter list
    Parms.Get(unicode('SHAPES')).Set_Value(use_variable_of_dataset_here) # input NOT optional shapes
    ...
    Parms.Get(unicode('TARGET_USER_COLS')).Set_Value(101)
    Parms.Get(unicode('TARGET_USER_ROWS')).Set_Value(101)
    Parms.Get(unicode('TARGET_USER_FITS')).Set_Value(0)
    Parms.Get(unicode('QUADRATIC_NEIGHBORS')).Set_Value(13)
    Parms.Get(unicode('WEIGHTING_NEIGHBORS')).Set_Value(19)
